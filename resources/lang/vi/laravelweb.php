<?php
return [
    'xinchao'=>"Xin chào ",
    'dangnhap'=>"ĐĂNG NHẬP",
    'dangky'=>"ĐĂNG KÝ",
    'dangxuat'=>"ĐĂNG XUẤT",


    'home' => "TRANG CHỦ",
    'products' => "SẢN PHẨM",
    'contact' => "LIÊN HỆ",
    'search' => "Tìm kiếm sản phẩm...",
    'loadmore' => "XEM THÊM",
    'inspiration' => "CẢM HỨNG",

    //contact page
    'message_title'=>"Gửi lời nhắn đến Houzie",
    'contactForm_email'=>"Vui lòng nhập gmail",
    'contactForm_mess'=>"Vui lòng nhập lời nhắn",
    'contactForm_emailpattern'=>"Chỉ chấp nhập GMAIL, vui lòng kiểm tra lại",
    'youremail'=>"Email của bạn",
    'needhelp'=>"Bạn cần chúng tôi giúp đỡ gì? Xin để lại lời nhắn",
    'diachi'=>"Địa chỉ",
    'diachi_ten'=>"KDC 515, đường 30 Tháng 4, Phường Hưng Lợi, Quận Ninh Kiều, TP Cần Thơ",
    'hotline'=>"Đường dây nóng",
    'support_email'=>"Email hỗ trợ",
    'submit'=>"GỬI LỜI NHẮN",
    //product-list
    'xemchitiet'=>'Xem chi tiết',

    //ngCart/sidebar-cart
    'tongtien'=>'Tổng thành tiền',

    // Cart
    'giohang' => 'Giỏ hàng',

     // Thanh Footer
     'thongtin' => 'Thông tin về Houzie',
     'dayla' => 'Chúng tôi là Houzie',
     'phongmoi' => 'Phòng mới',
     'ytuong' => 'Ý tưởng',
     'dayla' => 'Chúng tôi là Houzie',
     'trogiup' => 'TRỢ GIÚP',
     'theodoidonhang' => 'Theo dõi đơn hàng',
     'hoantra' => 'Chính sách hoàn trả',
     'vanchuyen' => 'Vận chuyển',
     'faqs' => 'Câu hỏi thường gặp',
     'lienhe' => 'LIÊN HỆ',
     'lienhecuthe' => 'Nếu có bất kì câu hỏi nào? Hãy liên hệ với chúng tôi tại KDC 515, đường 30 Tháng 4, Phường Hưng Lợi, Quận Ninh Kiều, TP Cần Thơ hoặc gọi cho chúng tôi theo số 0328-400-007',
     'bantin' => 'BẢN TIN',
     'dangkybantin' => 'Đăng ký',
     'lienhetoi' => 'Liên hệ chúng tôi',

      //Trang order-finish
      'order_finish' => 'Đặt hàng hoàn tất',
      'thanks' => 'Chúng tôi đã gởi email xác nhận đơn hàng cho Quý khách. Quý khách vui vòng kiểm tra hộp thư. Xin cám ơn Quý khách đã tin tưởng sản phẩm của chúng tôi.',
      'hotro' => 'Nếu cần hỗ trợ, vui lòng gọi đến đường dây nóng của chúng tôi để được hỗ trợ khi cần thiết:
      TEL: 0328400007',
];
?>